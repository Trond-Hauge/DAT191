...
export default async function getDocumentById(req: NextApiRequest, res: NextApiResponse) {
    if (req.method === "GET") {
        ...
        const doc = await db.select("documents.document_name", "documents.owner", "documents.document_description", "documents.filename", "documents.public","members.first_name", "members.last_name", "organisations.organisation_name")
            .from("documents")
